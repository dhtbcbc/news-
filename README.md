...
            <button class="close-modal-btn" onclick="closePoetModal()">×</button>
...
                © <span id="current-year"></span> كرار حيدر. جميع الحقوق محفوظة.
            </p>
...
                categoryButton.onclick = () => displayPoems(poet.poems[category]);
                poemCategoriesDiv.appendChild(categoryButton);
            });

            // Display poems of the first category by default
            if (categories.length > 0) {
                displayPoems(poet.poems[categories[0]]);
                poemCategoriesDiv.children[0].classList.add('active'); // Highlight first category
            }
